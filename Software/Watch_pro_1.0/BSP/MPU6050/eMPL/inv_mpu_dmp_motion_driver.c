/** @file inv_mpu_dmp_motion_driver.c @brief MPU6050 DMP运动驱动实现 - DMP固件加载、FIFO配置、手势检测、计步器、四元数输出等功能
 *  @details 本文件实现MPU6050内部DMP（数字运动处理器）的所有控制功能。
 *           DMP固件镜像存储在 dmp_memory[] 数组中（3062字节）。
 *           关键功能：dmp_load_motion_driver_firmware(加载DMP固件)、
 *           dmp_enable_feature(使能DMP特性)、dmp_read_fifo(读取DMP FIFO数据)、
 *           dmp_set_orientation(设置传感器方向)、dmp_set_gyro_bias(设置陀螺仪偏置) */
/*
 $License:
