/** @file inv_mpu.c @brief InvenSense MPU陀螺仪I2C驱动核心实现 - 传感器初始化、寄存器配置、FIFO读取、DMP固件加载、自检等
 *  @details 本驱动基于InvenSense官方MPL库移植到STM32平台。
 *           支持MPU6050/MPU6500/MPU9150/MPU9250等器件。
 *           关键功能：mpu_init(初始化)、mpu_set_sensors(传感器使能)、
 *           mpu_read_fifo(读取FIFO数据)、mpu_load_firmware(加载DMP固件)、
 *           mpu_dmp_init(DMP初始化)、mpu_dmp_get_data(获取DMP姿态数据) */
/*
 $License:
