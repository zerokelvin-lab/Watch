/** @file ui_NFCCardPage.c
 *  @brief NFC卡包页面实现 - 显示三张卡片（校园卡、门禁卡、交通卡），每张卡有改写和选择按钮
 */
#include "../../ui.h"
...
/* ====== 变量定义 ====== */
lv_obj_t * ui_NFCCardPage; // 卡包页面对象

// 卡片1 - 校园卡（蓝色）
lv_obj_t * ui_Card1Panel; // 卡片1面板
lv_obj_t * ui_Card1Btn; // 卡片1图标按钮
lv_obj_t * ui_Card1icon; // 卡片1图标
lv_obj_t * ui_Card1Label; // 卡片1名称标签
lv_obj_t * ui_WriteBtn1; // 改写按钮
lv_obj_t * ui_WriteLabel1; // 改写标签
lv_obj_t * ui_ChooseBtn1; // 选择按钮
lv_obj_t * ui_ChooseLabel1; // 选择标签
...
void ui_event_WriteBtn1(lv_event_t * e)
{
	...
	{
		ui_mbox_create("\xE5\xBC\x80\xE5\x8F\x91\xE4\xB8\xAD"); // 开发中
	}
}
...
/**
 * @brief  NFC卡包页面初始化 - 创建三张NFC卡面板，每张含图标、名称、改写和选择按钮
 */
void ui_NFCCardPage_screen_init(void)
{
    ...
    lv_obj_set_style_bg_color(ui_Card1Panel, lv_color_hex(0x0F80FF), LV_PART_MAIN | LV_STATE_DEFAULT); // 蓝色
    ...
    lv_label_set_text(ui_Card1icon, "\xEE\x99\x9A");
    ...
    lv_label_set_text(ui_Card1Label, "\xE6\xA0\xA1\xE5\x9B\xAD\xE5\x8D\xA1"); // 校园卡
    ...
    lv_label_set_text(ui_WriteLabel1, "\xE6\x94\xB9\xE5\x86\x99"); // 改写
    ...
    lv_label_set_text(ui_ChooseLabel1, "\xE9\x80\x89\xE6\x8B\xA9"); // 选择
    ...
    lv_obj_set_style_bg_color(ui_Card2Panel, lv_color_hex(0x0F800F), LV_PART_MAIN | LV_STATE_DEFAULT); // 绿色
    ...
    lv_label_set_text(ui_Card2icon, "\xEE\x98\x99");
    ...
    lv_label_set_text(ui_Card2Label, "\xE9\x97\xA8\xE7\xA6\x81\xE5\x8D\xA1"); // 门禁卡
    ...
    lv_label_set_text(ui_WriteLabel2, "\xE6\x94\xB9\xE5\x86\x99"); // 改写
    ...
    lv_label_set_text(ui_ChooseLabel2, "\xE9\x80\x89\xE6\x8B\xA9"); // 选择
    ...
    lv_obj_set_style_bg_color(ui_Card3Panel, lv_color_hex(0x0F80FF), LV_PART_MAIN | LV_STATE_DEFAULT); // 蓝色
    ...
    lv_label_set_text(ui_Card3icon, "\xEE\x98\x82");
    ...
    lv_label_set_text(ui_Card3Label, "\xE4\xBA\xA4\xE9\x80\x9A\xE5\x8D\xA1"); // 交通卡
    ...
    lv_label_set_text(ui_WriteLabel3, "\xE6\x94\xB9\xE5\x86\x99"); // 改写
    ...
    lv_label_set_text(ui_ChooseLabel3, "\xE9\x80\x89\xE6\x8B\xA9"); // 选择
