/** @file ui_Game2048Page.c
 *  @brief 2048游戏页面实现 - 经典数字合并游戏，手势滑动控制移动方向，相同数字合并并累加得分
 */
#include "stdio.h"
#include "stdlib.h"
#include "string.h"
...
/* ====== 游戏宏定义 ====== */
#define STACK_DEPTH 5 // 合并栈深度
#define MATRIX_SIZE 4 // 游戏矩阵大小（4x4）

// 数字栈结构（用于合并逻辑）
typedef struct
{
    uint16_t Data[STACK_DEPTH]; // 栈数据
...
typedef struct {
    uint16_t score; // 当前得分
    uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]; // 游戏矩阵（存储2的幂次指数）
    char    * btnm_map[MATRIX_SIZE * MATRIX_SIZE + MATRIX_SIZE]; // 按钮矩阵映射
...
static char *Str2048Nums[]={"2","4","8","16","32","64","128","256","512","1024","2048"}; // 数字字符串

/* ====== 游戏函数声明 ====== */
static void Game_2048_init(void); // 游戏初始化
static void init_matrix_num(uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 初始化矩阵
static void update_btnm_map(char * btnm_map[], uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 更新按钮映射
static void addRandom(uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 随机添加新数字
static uint8_t move_up(uint16_t * score, uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 向上移动
static uint8_t move_down(uint16_t * score, uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 向下移动
static uint8_t move_right(uint16_t * score, uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 向右移动
static uint8_t move_left(uint16_t * score, uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 向左移动
static uint8_t game_over(uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE]); // 判断游戏结束
static lv_color_t get_num_color(uint16_t num); // 获取数字对应颜色

/* ====== 页面管理器 ====== */
Page_t Page_Game_2048 = {ui_Game2048Page_screen_init, ui_Game2048Page_screen_deinit, &ui_Game2048Page};

/* ====== 变量定义 ====== */
ui_Game2048_t Game_2048; // 2048游戏状态

lv_obj_t * ui_Game2048Page; // 游戏页面对象
lv_obj_t * ui_Game2048BtnM; // 游戏按钮矩阵
lv_obj_t * ui_Game2048ScLabel; // 分数标签

/**
 * @brief  游戏页面手势事件 - 根据手势方向移动方块
 * @param  e LVGL事件对象
 */
static void ui_event_Game2048Page(lv_event_t * e)
{
   lv_event_code_t event_code = lv_event_get_code(e);
    ...
    {
				Game_2048.game_over = game_over(Game_2048.matrix); // 先检查游戏是否结束
...
static void ui_event_Game2048BtnM(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_DRAW_PART_BEGIN)
    {
        ...
        if((dsc->id >= 0) && (dsc->label_dsc))
        {
            ...
            x = (uint16_t)((dsc->id) / MATRIX_SIZE); // 计算行
            y = (dsc->id) % MATRIX_SIZE; // 计算列
            num = (uint16_t)(1 << (Game_2048.matrix[x][y])); // 将指数转换为实际数值（2^n）

            dsc->rect_dsc->bg_color = get_num_color(num); // 根据数值设置背景色
            if (num > 516) // 大数字用黑色文字
...
/**
 * @brief  2048游戏页面初始化 - 创建按钮矩阵、分数标签和重新开始按钮
 */
void ui_Game2048Page_screen_init(void)
{
    ...
    lv_label_set_text(new_game_btn_label,"\xE9\x87\x8D \xE7\x8E\xA9"); // 重玩按钮
...
/**
 * @brief  游戏数据初始化 - 重置分数、清空矩阵、添加初始数字
 */
static void Game_2048_init(void)
{
    Game_2048.score = 0; // 重置分数
    Game_2048.game_over = 0; // 清除结束标志

    memset(Game_2048.btnm_map,0,MATRIX_SIZE * MATRIX_SIZE + MATRIX_SIZE); // 清空按钮映射

    // 设置换行符位置（4x4矩阵，每行末尾换行）
    Game_2048.btnm_map[4]="\n";
    Game_2048.btnm_map[9]="\n";
    Game_2048.btnm_map[14]="\n";
    Game_2048.btnm_map[19]="";

    init_matrix_num(Game_2048.matrix); // 初始化矩阵
    update_btnm_map(Game_2048.btnm_map, Game_2048.matrix);
    lv_btnmatrix_set_map(ui_Game2048BtnM, Game_2048.btnm_map);
...
static void update_btnm_map(char * btnm_map[], uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE])
{
    ...
    for (x = 0; x < MATRIX_SIZE; x++)
    {
        for (y = 0; y < MATRIX_SIZE; y++)
        {
            ...
            if(((index + 1) % 5) == 0) // 跳过换行符位置
                index++;

            if(matrix[x][y] != 0)
            {
                btnm_map[index] = Str2048Nums[matrix[x][y]-1]; // 显示对应数字
            }
            else
                btnm_map[index] = " "; // 空格表示空位
...
/**
 * @brief  随机添加新数字 - 在空位上随机放置2或4
 * @param  matrix 游戏矩阵
 */
static void addRandom(uint16_t matrix[MATRIX_SIZE][MATRIX_SIZE])
{
    static uint8_t initialized = 0; // 随机数种子初始化标志
    uint16_t x, y;
    uint16_t r, len = 0;
    ...
    if (!initialized)
		{
			...
			time = nowtime.Seconds; // 用当前秒数作为随机种子
      srand(time);
      initialized = 1;
    }
    ...
    if (len > 0) {
        ...
        x = list[r][0];
        y = list[r][1];
        n = ((rand() % 10) / 9) + 1; // 90%概率为2，10%概率为4
        matrix[x][y] = n;
    }
}

/**
 * @brief  向上移动 - 将所有数字向上移动并合并相同数字
 * @param  score  得分指针
 * @param  matrix 游戏矩阵
 * @return 是否有移动发生
 */
    ...
    for (y = 0; y < MATRIX_SIZE; y++)
    {
        ...
        for(x=0; x<MATRIX_SIZE; x++)
        {
            ...
            if(num_stack.Top_Point>1)
            {
                ...
                {
                    uint16_t temp;
                    temp = num_stack.Data[num_stack.Top_Point-1] + 1; // 合并后指数+1
                    Stack_Pop(&num_stack);
                    Stack_Pop(&num_stack);
                    Stack_Push(&num_stack,temp);
                    *score += 1<<(temp-1); // 累加得分（2^temp）
                    moved = 1;
                }
            }
        }
...
/**
 * @brief  判断游戏结束 - 检查是否还有空位
 * @param  matrix 游戏矩阵
 * @return 1=游戏结束，0=游戏继续
 */
