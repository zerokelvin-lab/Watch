/** @file ui_DateTimeSetPage.c
 *  @brief 日期时间设置页面实现 - 包含APP同步开关、日期设置（年/月/日滚轮）和时间设置（时/分/秒滚轮）三个子页面
 */
#include "../../ui.h"
...
/* ====== 变量定义 ====== */
lv_obj_t * ui_DateTimeSetPage; // 日期时间设置主页
lv_obj_t * ui_APPSyPanel; // APP同步面板
lv_obj_t * ui_APPSySwitch; // APP同步开关
lv_obj_t * ui_APPSyLabel; // APP同步标签
lv_obj_t * ui_DateSetPanel; // 日期设置入口面板
lv_obj_t * ui_DateSetLabel; // 日期设置主标签
lv_obj_t * ui_DateSetLabel1; // 装饰点
lv_obj_t * ui_DateSetLabel2; // 装饰点
lv_obj_t * ui_DateSetLabel3; // 装饰点
lv_obj_t * ui_TimeSetPanel; // 时间设置入口面板
lv_obj_t * ui_TimeSetLabel; // 时间设置主标签
lv_obj_t * ui_TimeSetLabel1; // 装饰点
lv_obj_t * ui_TimeSetLabel2; // 装饰点
lv_obj_t * ui_TimeSetLabel3; // 装饰点

lv_obj_t * ui_DateSetPage; // 日期设置子页面
lv_obj_t * ui_YearSetRoller; // 年份滚轮
lv_obj_t * ui_MonthSetRoller; // 月份滚轮
lv_obj_t * ui_DaySetRoller; // 日期滚轮
lv_obj_t * ui_DateSetOKButton; // 日期确认按钮
lv_obj_t * ui_DateSetOKicon; // 日期确认图标

lv_obj_t * ui_TimeSetPage; // 时间设置子页面
lv_obj_t * ui_HourSetRoller; // 小时滚轮
lv_obj_t * ui_TimePoint; // 时:分冒号
lv_obj_t * ui_MinSetRoller; // 分钟滚轮
lv_obj_t * ui_TimePoint1; // 分:秒冒号
lv_obj_t * ui_SecSetRoller; // 秒滚轮
lv_obj_t * ui_TimeSetOKButton; // 时间确认按钮
lv_obj_t * ui_TimeSetOKicon; // 时间确认图标


uint8_t ui_APPSy_EN = 0; // APP同步使能标志
...
void ui_event_DateTimeSetPage(lv_event_t * e)
{
   lv_event_code_t event_code = lv_event_get_code(e);
    ...
    {
			if(lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_RIGHT) // 右滑手势
      {
        Page_Back(); // 返回上一页
      }
		}
...
void ui_event_DateSetPanel(lv_event_t * e)
{
    ...
    {
      Page_Load(&Page_DateSet); // 加载日期设置页面
    }
}

/**
 * @brief  时间设置入口点击事件 - 跳转到时间设置子页面
 * @param  e LVGL事件对象
 */
void ui_event_TimeSetPanel(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_CLICKED)
    {
      Page_Load(&Page_TimeSet); // 加载时间设置页面
    }
}

/**
 * @brief  日期确认按钮事件 - 将滚轮选中的年/月/日写入RTC
 * @param  e LVGL事件对象
 */
void ui_event_DateSetOKButton(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_CLICKED)
    {
			uint8_t setyear = lv_roller_get_selected(ui_YearSetRoller)+22; // 年份偏移22年（从2022开始）
			uint8_t setmonth = lv_roller_get_selected(ui_MonthSetRoller)+1; // 月份从1开始
			uint8_t setday = lv_roller_get_selected(ui_DaySetRoller)+1; // 日期从1开始

      HWInterface.RealTimeClock.SetDate(setyear, setmonth, setday); // 设置RTC日期
			ui_DateMonthValue = setmonth; // 同步主页月份
			ui_DateDayValue = setday; // 同步主页日期
			ui_DataWeekdayValue =  HWInterface.RealTimeClock.CalculateWeekday(setyear,setmonth,setday,20); // 计算星期
...
/**
 * @brief  时间确认按钮事件 - 将滚轮选中的时/分/秒写入RTC
 * @param  e LVGL事件对象
 */
void ui_event_TimeSetOKButton(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_CLICKED)
    {
      ...
      HWInterface.RealTimeClock.SetTime(sethour,setmin,setsec); // 设置RTC时间
			ui_TimeHourValue = sethour; // 同步主页小时
			ui_TimeMinuteValue = setmin; // 同步主页分钟
...
/**
 * @brief  APP同步开关事件 - 开启/关闭APP时间同步功能
 * @param  e LVGL事件对象
 */
...
/**
 * @brief  日期时间设置主页初始化 - 创建APP同步开关、日期/时间设置入口面板
 */
...
/**
 * @brief  日期设置子页面初始化 - 创建年/月/日三个滚轮和确认按钮
 */
void ui_DateSetPage_screen_init(void)
{

    HW_DateTimeTypeDef datetime;
    HWInterface.RealTimeClock.GetTimeDate(&datetime); // 读取RTC获取当前日期
    ...
    lv_roller_set_selected(ui_YearSetRoller, datetime.Year-22, LV_ANIM_OFF); // 年份偏移
    ...
    lv_obj_set_style_outline_color(ui_YearSetRoller, lv_color_hex(0x0064FF), LV_PART_SELECTED | LV_STATE_DEFAULT); // 选中项蓝色边框
    ...
    lv_label_set_text(ui_DateSetOKicon, "\xEE\xB8\x8D"); // 确认图标
...
/**
 * @brief  时间设置子页面初始化 - 创建时/分/秒三个滚轮和确认按钮
 */
void ui_TimeSetPage_screen_init(void)
{

    HW_DateTimeTypeDef datetime;
    HWInterface.RealTimeClock.GetTimeDate(&datetime); // 读取RTC获取当前时间
    ...
    lv_label_set_text(ui_TimeSetOKicon, "\xEE\xB8\x8D"); // 确认图标
