/** @file ui_Megboxes.c
 *  @brief 消息弹框组件实现 - 通用警告对话框，含警告图标、提示文字及OK/Cancel按钮
 */
#include "../../ui.h"
#include "../../ui_helpers.h"

/* ====== 变量定义 ====== */
lv_obj_t * ui_mboxPanel; // 消息框面板
lv_obj_t * ui_WarningSymbol; // 警告图标
lv_obj_t * ui_WarningLabel; // 警告文字
lv_obj_t * ui_ApplyPanel; // 确认面板
lv_obj_t * ui_ApplyLabel; // 确认标签
lv_obj_t * ui_CancelPanel; // 取消面板
lv_obj_t * ui_CancelLabel; // 取消标签

/* ====== 功能函数 ====== */

/**
 * @brief  关闭消息框 - 删除消息框并恢复父页面滚动
 */
void ui_mbox_close(void)
{
    lv_obj_del(ui_mboxPanel); // 删除消息框对象
    lv_obj_add_flag(lv_scr_act(),LV_OBJ_FLAG_SCROLLABLE); // 恢复屏幕滚动
}

/**
 * @brief  确认按钮事件回调 - 关闭消息框
 * @param  e LVGL事件对象
 */
...
/**
 * @brief  创建消息框 - 在当前屏幕上创建模态对话框
 * @param  text 警告提示文字
 */
void ui_mbox_create(uint8_t * text)
{
    lv_obj_clear_flag(lv_scr_act(), LV_OBJ_FLAG_SCROLLABLE); // 禁用屏幕滚动
    ...
    lv_label_set_text(ui_WarningLabel, text); // 显示传入的文字
    ...
    lv_obj_set_style_border_side(ui_ApplyPanel, LV_BORDER_SIDE_TOP, LV_PART_MAIN | LV_STATE_DEFAULT); // 仅上边框
    ...
    lv_obj_set_style_text_color(ui_ApplyLabel, lv_color_hex(0x5FB878), LV_PART_MAIN | LV_STATE_DEFAULT); // 绿色
    ...
    lv_obj_set_style_text_color(ui_CancelLabel, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT); // 红色
